...
// Definimos el tipo de dato basado en tu tabla de Supabase
type Restaurant = {
  id: string;
  name: string;
  address: string;
  ...
  const fetchRestaurants = async () => {
    try {
      const { data, error } = await supabase
        .from("restaurants")
        .select("*")
        .eq("is_open", true); // Solo traemos los que están abiertos
  ...
  return (
    ...
    >
      <ScrollView className="flex-1" showsVerticalScrollIndicator={false}>
        ...
        {/* GREETING & SEARCH BAR... (Mismo código de antes) */}
        ...
        {/* FEATURED SECTION - AHORA CON DATOS REALES */}
